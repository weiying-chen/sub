...
describe("fillSelectedTimestampLines", () => {
  ...
  it("does not emit a standalone proper name cue before a continuation clause", () => {
  const lines = [
    "00:00:24:29\t00:00:26:22\t我跟李寶春合作在一起",
    "00:00:26:22\t00:00:27:12\t我們算了一下",
    "00:00:27:12\t00:00:28:25\t八十幾檔戲",
  ]
  const selected = new Set([0, 1, 2])
  ...
  const result = fillSelectedTimestampLines(
    lines,
    selected,
    "Li Baochun and I have done over 80 productions together.",
    { inline: true }
  )

  const translations = result.lines.filter((line) => !line.includes("\t"))
  expect(translations).not.toContain("Li Baochun")
  expect(translations.some((line) => /^Li Baochun and I\b/.test(line))).toBe(true)
  })

  it("avoids repeated partial duplicates when a fuller duplicate follows", () => {
  const lines = [
    "00:01:20:12\t00:01:21:15\t票價很不貴",
    "00:01:21:15\t00:01:22:28\t因為大家還要跑出來",
    "00:01:22:28\t00:01:25:06\t要不然你就坐在家裡",
    "00:01:25:06\t00:01:26:17\t你就看",
    "00:01:26:17\t00:01:27:28\t就做直播",
  ]
  const selected = new Set([0, 1, 2, 3, 4])
  ...
  const result = fillSelectedTimestampLines(
    lines,
    selected,
    "Tickets have to stay affordable, otherwise people would just watch the livestreams at home.",
    { inline: true }
  )

  const translations = result.lines.filter((line) => !line.includes("\t"))
  const partial = "Tickets have to stay affordable, otherwise people"
  const full = "would just watch the livestreams at home."
  expect(translations.filter((line) => line === partial).length).toBeLessThan(2)
  expect(translations.filter((line) => line === full).length).toBeGreaterThan(0)
  })

  it("avoids repeated partial duplicates when lead-in has no trailing comma", () => {
  const lines = [
    "00:01:27:28\t00:01:29:13\t一方面讓年輕人",
    "00:01:29:13\t00:01:31:01\t終於有演出的機會",
    "00:01:31:01\t00:01:32:04\t一方面",
    "00:01:32:04\t00:01:34:18\t就磨練他們的舞臺",
    "00:01:34:18\t00:01:37:16\t跟在攝影機前面看著",
    "00:01:37:16\t00:01:38:22\t做戲的能力",
  ]
  const selected = new Set([0, 1, 2, 3, 4, 5])
  ...
  const result = fillSelectedTimestampLines(
    lines,
    selected,
    "It gave the younger performers a chance to get onstage while helping them develop both stage and on-camera skills.",
    { inline: true }
  )

  const translations = result.lines.filter((line) => !line.includes("\t"))
  const partial = "It gave the younger performers a chance to get onstage"
  const full = "while helping them develop both stage"
  expect(translations.filter((line) => line === partial).length).toBeLessThan(2)
  expect(translations.filter((line) => line === full).length).toBeGreaterThan(0)
  })

  it("does not introduce orphan tail fragments during backfill", () => {
  const lines = [
    "00:01:36:09\t00:01:37:06\t因為有些人",
    "00:01:37:06\t00:01:39:01\t比我們比較不幸",
    "00:01:39:01\t00:01:40:26\t或者是說有些人",
    "00:01:41:26\t00:01:43:03\t比我們沒有機會",
    "00:01:43:03\t00:01:45:02\t去接觸這麼美麗的藝術",
    "00:01:45:02\t00:01:46:27\t我們覺得有點可惜",
  ]
  const selected = new Set([0, 1, 2, 3, 4, 5])
  ...
  const result = fillSelectedTimestampLines(
    lines,
    selected,
    "After all, some people just haven't had the same opportunities we have. Some people never had the chance",
    { inline: true }
  )

  const translations = result.lines.filter((line) => !line.includes("\t"))
  expect(translations).not.toContain("Some people never had the chance")
  })
})
