...
describe("fill-subs executable entry", () => {
  it("uses a symlink-safe, cwd-independent tsx launcher shebang", () => {
    const source = readFileSync(CLI_FILE_URL, "utf8")
    const firstLine = source.split(/\r?\n/, 1)[0]

    expect(firstLine.startsWith("#!")).toBe(true)
    expect(firstLine).toContain("sh -c")
    expect(firstLine).toContain("readlink -f")
    expect(firstLine).toContain("dirname")
    expect(firstLine).toContain("node_modules/.bin/tsx")
  })
